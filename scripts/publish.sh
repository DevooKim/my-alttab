...
ZIP="dist/My-AltTab-v$VERSION.zip"
...
{
    echo "**Install:** unzip, move \`My AltTab.app\` to Applications, open via System Settings > Privacy & Security (\"Open Anyway\"), then grant Accessibility permission. See the [README](README.md#install) / [한국어 안내](README.ko.md#설치)."
    echo
    echo "## Changes"
    echo
    if [ -n "$PREV_TAG" ]; then
        # Commit subjects since the last release; version-bump chores are noise.
        git log "$PREV_TAG"..HEAD --no-merges --pretty="- %s" | grep -v "^- chore: bump version"
        echo
        echo "**Full Changelog**: https://github.com/DevooKim/my-alttab/compare/$PREV_TAG...v$VERSION"
    else
        git log --no-merges --pretty="- %s" | grep -v "^- chore: bump version"
    fi
} > "$NOTES_FILE"
