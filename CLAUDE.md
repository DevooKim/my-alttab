...
- **`kCGWindowName` is gated behind the opt-in "show all Spaces" setting** (`Preferences.showAllSpaces`, off by default). It silently requires Screen Recording permission, so it is read ONLY in `WindowEnumerator.inactiveSpaceWindows` for windows on inactive Spaces (which AX cannot reach). With the setting off, the app never touches `kCGWindowName`. Active-Space titles always come from AX (`kAXTitleAttribute`); `CGWindowListCopyWindowInfo` is otherwise used only for PID z-ordering (permission-free).
- **Space (virtual desktop) support uses private CGS APIs**, isolated in `System/SpaceTracker.swift` (`_AXUIElementGetWindow`, `CGSCopySpacesForWindows`, `CGSCopyManagedDisplaySpaces`, `CGSCopyWindowsWithOptionsAndTags`). AX only returns the active Space's windows, so cross-Space listing depends on these. They read window IDs/topology only — no contents.
